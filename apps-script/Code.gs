...
//  v2 — Lectura + Escritura de pedidos
...
function buildPayload(ss) {
  ...
  // Helper: log qué hoja se encontró para cada clave
  function findAndLog(label, keywords) {
    var sheet = findSheet(sheets, keywords);
    Logger.log(label + ': ' + (sheet ? sheet.getName() : 'NO ENCONTRADA') + ' (buscando: ' + keywords.join(', ') + ')');
...
function parseSheet(sheet) {
  ...
  var headers   = data[headerIdx].map(function(h){
    return String(h).trim().replace(/\r?\n/g, ' ').replace(/\s+/g, ' '); // colapsa dobles espacios
  });
...
function updatePedido(ss, params) {
  ...
  // Buscar la fila que coincide con el número de orden
  var targetRow = -1;
  for (var r = headerIdx + 1; r < data.length; r++) {
  ...
  // Actualizar cada columna si viene el parámetro
  var updated = [];
...
function addPedido(ss, params) {
  ...
  // Armar la nueva fila según columnas de la hoja
  ...
  // Insertar en la primera fila vacía después de los datos
